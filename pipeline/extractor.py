    ...
    for file_path in directory_path.rglob('*'):
        if file_path.is_file():
            if file_path.suffix.lower() == '.pdf':
                extracted_texts[str(file_path.relative_to(directory_path))] = extract_text_from_pdf(file_path)
            elif file_path.suffix.lower() == '.txt':
                extracted_texts[str(file_path.relative_to(directory_path))] = extract_text_from_txt(file_path)
            elif file_path.suffix.lower() == '.md':
                extracted_texts[str(file_path.relative_to(directory_path))] = extract_text_from_md(file_path)
            elif file_path.suffix.lower() == '.zip':
                zip_texts = process_zip_file(file_path)
                for filename, text in zip_texts.items():
                    extracted_texts[f"{file_path.relative_to(directory_path)}/{filename}"] = text
